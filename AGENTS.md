...
- `provider.ts`
  ...
  - mirrors Codex models through current `@earendil-works/pi-ai` registry APIs
  - installs the managed stream wrapper
- `stream-wrapper.ts`
  - account selection, retry, and quota-rotation path during streaming
  - uses local stream and abort primitives from `stream-utils.ts`
- `account-manager.ts`
...
- `auth.ts`
  ...
  - resolves agent paths through local `agent-paths.ts`
- `status.ts`
...
- `storage.ts`
  ...
  - resolves storage path through local `agent-paths.ts`
- `selection.ts`
...
- Provider mirroring, stream primitives, and agent-path helpers are local so they stay aligned with current `@earendil-works/*` pi package names.
